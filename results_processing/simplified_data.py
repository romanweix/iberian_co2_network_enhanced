...
cluster_map_df = pd.read_excel('simplified_model_exported_data.xlsx', sheet_name='Emissions clustering map')
...
candidate_pipelines_gdf = pd.read_excel('simplified_model_exported_data.xlsx', sheet_name='Pipeline candidates')  # Read candidate pipelines data from Excel file
...
complete_nodes_gdf = pd.read_excel('simplified_model_exported_data.xlsx', sheet_name='Nodes')  # Read nodes data from Excel file
...
rows = []                                           # ¡Importante: vaciar la lista!
...
# DataFrame limpio con solo las columnas deseadas
selling_revenues_5yr_df = (
    pd.DataFrame(rows)
      .astype({"Year": int})                        # asegúrate de tipo int
      .sort_values('Year')
      .reset_index(drop=True)
)
...
def build_input_dict(
    ...
    # Si la geometría está en texto WKT, conviértela a shapely Point ---------
    if isinstance(nodes.geometry.iloc[0], str):        # ← detectamos que es str
        nodes["geometry"] = nodes["geometry"].apply(wkt.loads)

    # Aseguramos que 'nodes' es un GeoDataFrame con la columna geometry
    ...
    # --- coordenadas (lon, lat) -----------------------------------------
    # usamos el GeoDataFrame original ('nodes'), no '_nodes'
    coords = {
    ...
    # node_emissions_5yr_df contiene las emisiones (MtCO2) de cada Source Cluster
    # en los time-steps de 5 años. Se mapea Source Cluster → Node identifier.

    emis: Dict[Tuple[Node, Year], float] = {}

    # Si el DataFrame llega con otro nombre, ajusta esta línea:
    ...
    # Construimos un mapping robusto: el Source Cluster 0 se corresponde con el primer
    # node_id de tipo 'E', el 1 con el segundo, etc.
    ...
    for _, row in node_emissions_5yr_df.iterrows():
        ...
        if n is None:
            # El cluster no tiene nodo 'E' equivalente; lo ignoramos
            continue
        ...
        for t in years:
            ...
            emis[(n, t)] = float(row[col])   # MtCO2 emitidos en el bloque de 5 años
    ...
    # ----------------------------------------------------------
    # helper: lista of nodes M per country
    country_to_nodes: Dict[str, List[int]] = {}
    for n in M:                               # M is already defined
        ctry = country_n[n]                   # country_n is {node: "France", …}
        if ctry in ["France", "Morocco", "Argelia"]:
            country_to_nodes.setdefault(ctry, []).append(n)

    # 15) Trading prices ---------------------------------------
    import_price, export_price = {}, {}

    for _, r in pv_trading_prices_df.iterrows():
        yr = int(r["Year"])
        if yr not in years:
            continue
        for c in country_to_nodes:
            rev_col  = f"Importing revenue from {c}"
            cost_col = f"Importing cost to {c}"
            raw_rev  = r.get(rev_col, 0)
            raw_cost = r.get(cost_col, 0)
            for n in country_to_nodes[c]:
                import_price[(n, yr)] = 0.0 if pd.isna(raw_rev)  else float(raw_rev)
                export_price[(n, yr)] = 0.0 if pd.isna(raw_cost) else float(raw_cost)

    # 16) Max trade fraction -----------------------------------
    trade_frac = {}
    for _, r in max_trade_flows_df.iterrows():
        yr = int(r["Year"])
        if yr not in years:
            continue
        for c in country_to_nodes:
            frac_val = 0.0 if pd.isna(r.get(c, 0)) else float(r[c])
            for n in country_to_nodes[c]:
                trade_frac[(n, yr)] = frac_val

    # 17) Selling revenue (industrial use) --------------------------------
    ...
    # 18) Partición P1/P2: P2 si el pipe toca un nodo K ---
    ...
    # --- Escenarios desde scenarios.py ---
    cfg = dict(scenarios_cfg or {})
    cfg.setdefault("nodes_uncertain", list(K))   # por defecto, todos los K
    cfg.setdefault("time_steps", list(years))    # tus T
    ...
    data: Dict[str, Any] = {
        ...
        # Trading
        "import_price": import_price, "export_price": export_price, "trade_frac": trade_frac,
    ...
    assert all((node in M) for node, _ in trade_frac), \
        "Found trade_frac node not in set M"
...
#     # Falla si algún DataFrame no existe todavía
#     missing = [v for v in (
#         "cand_pipe_features_gdf", "pipe_diam_features_df", "complete_nodes_gdf",
